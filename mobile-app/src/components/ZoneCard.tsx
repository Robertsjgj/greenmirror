...
export function ZoneCard({ zone, assignedPlant, onSelect }: ZoneCardProps) {
  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border p-2 text-left transition-transform active:scale-[0.98] ${tone.card}`}
    >
      ...
      <p className={`mt-2 truncate text-[9px] font-bold uppercase tracking-wider ${tone.text}`}>
        {status.label}
      </p>
    </button>
  );
