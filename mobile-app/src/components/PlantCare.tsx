...
function loadCompletedTaskIds(ghId: string): Set<string> {
  try {
    ...
    return new Set<string>(
      Array.isArray(data[SESSION_DATE]) ? data[SESSION_DATE] : [],
    );
  } catch {
    return new Set<string>();
  }
...
function persistCompletedTask(ghId: string, taskId: string): void {
  try {
    ...
    Object.entries(data).forEach(([d, ids]) => {
      if (d >= cutoffStr) pruned[d] = ids;
    });
    pruned[SESSION_DATE] = [...todaySet];
    window.localStorage.setItem(
      completedTasksKey(ghId),
...
function buildTask(z: VisualZone, profile: PlantProfile | null): Task | null {
  ...
  if (!["needs-water", "too-wet", "too-cold", "too-hot"].includes(status))
    return null;
...
export function PlantCare({
  ...
  const allTasks = useMemo(() => {
    ...
    return zones
      .map((z) =>
        buildTask(
          z,
          z.assignedPlant ? (profilesById.get(z.assignedPlant) ?? null) : null,
        ),
      )
  ...
  const summary = useMemo(() => {
    let good = 0,
      attention = 0;
    zones.forEach((z) => {
      const ev = evaluateZoneAgainstPlant(
        z,
        z.assignedPlant ? (profilesById.get(z.assignedPlant) ?? null) : null,
      );
  ...
  const assignedZonesByPlant = useMemo(() => {
    ...
    zones.forEach((z) => {
      if (z.assignedPlant)
        m[z.assignedPlant] = [...(m[z.assignedPlant] ?? []), z];
    });
    return m;
  }, [zones]);
  ...
  const pagedProfiles = filteredProfiles.slice(
    safePage * PROFILES_PER_PAGE,
    (safePage + 1) * PROFILES_PER_PAGE,
  );
      ...
      {!loading &&
        ...
        (needsAttention === 0 ? (
          ...
          <div className="gm-callout">
            ...
            <div>
              <h3>
                Greenhouse: {needsAttention} section
      ...
      <div>
        ...
        >
          <div>
            ...
            <div className="gm-sub">
              {plantProfiles.length} kinds saved · tap one to edit
            </div>
              ...
              {totalPages > 1 && (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setProfilePage((p) => Math.min(totalPages - 1, p + 1))
                    }
                  >
                    Next ›
                  </button>
        ...
        >
          Profiles say what each plant likes. Assign one in the Map — change it
          any time! 🪴
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        <span
          className="gm-chip wet"
          style={{ background: "var(--wet-soft)", color: "var(--wet)" }}
        >
          💧 {profile.moistureMin}–{profile.moistureMax}%
        </span>
        <span
          className="gm-chip dry"
          style={{ background: "var(--dry-soft)", color: "oklch(0.5 0.13 65)" }}
        >
      ...
      {zoneCount > 0 && (
        ...
        >
          Assigned to {assignedLabels}
          {zoneCount > 4 ? ` +${zoneCount - 4} more` : ""}
        </div>
      )}
    </button>
  );
